%% Run MATLAB Benchmark Tests from correct directory
...
fprintf('MATLAB MOCAT-MC Benchmark Test Suite\n');
fprintf('%s\n', repmat('=', 1, 50));

% Define tests matching Python exactly
tests = {
    'Basic Propagation', 42, 5, @basic_propagation_config;
    'Collision Test', 123, 10, @collision_test_config;
    'Atmospheric Drag', 333, 10, @atmospheric_drag_config;
    'Full Default', 1, 75, @full_default_config;
};
...
for i = 1:size(tests, 1)
    test_name = tests{i, 1};
    seed = tests{i, 2};
    expected_steps = tests{i, 3};
    config_func = tests{i, 4};
...
fprintf('\n%-20s %-10s %-8s %-8s %-6s %-6s %-6s %-6s %-8s\n', ...
    'Test', 'Status', 'Initial', 'Final', 'S', 'D', 'N', 'B', 'Time(s)');
fprintf('%s\n', repmat('-', 1, 85));

for i = 1:length(results)
    r = results(i);
    if r.success
        fprintf('%-20s %-10s %-8d %-8d %-6d %-6d %-6d %-6d %-8.2f\n', ...
...
% Export to JSON for easy Python reading
json_results = {};
for i = 1:length(results)
    r = results(i);
    if r.success
        json_results{i} = struct( ...
            'test_name', r.test_name, ...
            'seed', r.seed, ...
            'success', r.success, ...
            'initial_pop', r.initial_pop, ...
            'final_pop', r.final_pop, ...
            'nS', r.nS, ...
            'nD', r.nD, ...
            'nN', r.nN, ...
            'nB', r.nB, ...
            'satellite_ratio', r.satellite_ratio, ...
            'elapsed_time', r.elapsed_time, ...
            'n_time_steps', r.n_time_steps ...
        );
    end
end
...
fid = fopen('matlab_benchmark_results.json', 'w');
fprintf(fid, '%s', json_str);
fclose(fid);
fprintf('Results also saved to matlab_benchmark_results.json\n');
