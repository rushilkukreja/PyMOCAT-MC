...
def mean_osculating_map(x, option, param):
    ...
    term1 = ((3 * np.cos(inc)**2 - 1) / (eta**6)) * (e * eta + (e / (1 + eta)) + 
                                                     3 * np.cos(theta) + 3 * e * np.cos(theta)**2 + e**2 * np.cos(theta)**3)
    term2 = (3 * (1 - np.cos(inc)**2) / (eta**6)) * (e + 3 * np.cos(theta) + 
                                                     3 * e * np.cos(theta)**2 + e**2 * np.cos(theta)**3) * np.cos(2 * omega + 2 * theta)
    ...
    Delta_inc = (-(e * Delta_e1) / (eta**2 * np.tan(inc)) + 
                 ...
                 (3 * np.cos(2 * omega + 2 * theta) + 3 * e * np.cos(2 * omega + theta) + 
                  e * np.cos(2 * omega + 3 * theta)))
    
    angle_total = Mo + omega + bigO
    angle_total_p = (angle_total +
                    (gam2_p / 8) * eta**3 * (1 - 11 * np.cos(inc)**2 - 40 * (np.cos(inc)**4 / (1 - 5 * np.cos(inc)**2))) +
                    -(gam2_p / 16) * (2 + e**2 - 11 * (2 + 3 * e**2) * np.cos(inc)**2 - 
    ...
    eDelta_M = ((gam2_p / 8) * e * eta**3 * (1 - 11 * np.cos(inc)**2 - 40 * (np.cos(inc)**4 / (1 - 5 * np.cos(inc)**2))) +
