...
def init_sim(cfg: Dict, simulation: str, launch_model: str, ic_file: str) -> Dict:
    """
    ...
    Args:
        ...
        launch_model: Launch model ('no_launch', 'random', 'matsat', 'data', 'Somma')
    ...
    try:
        ...
        if mat_data is None:
            raise FileNotFoundError(f"Could not find {ic_file} in any expected location")
        ...
        print(f'{mat_sats.shape[0]} satellite entries on {time0} loaded from {actual_path}')
    ...
    mat_sats[payload_mask, idx['controlled']] = 1  # Set controlled flag on for all payloads
    ...
    mat_sats[derelict_mask, idx['controlled']] = 0  # Set controlled flag off for derelicts
    ...
    if launch_model in ['no_launch', 'no']:
        ...
        if launch_model == 'matsat':
            # Create repeat launches based on past launch performance within the launch window
            launch_dates = mat_sats[:, idx['launch_date']]
                ...
                if valid_dates[i]:
                    try:
                        ...
                        ind_inlaunchwindow[i] = (
                            launch_year >= cfg['launchRepeatYrs'][0] and 
                            launch_year <= cfg['launchRepeatYrs'][1]
                        )
                    except:
            ...
            # Remove constellations if needed (Starlink: 260kg, OneWeb: 148kg)
            if len(repeat_launches) > 0:
                ...
                print(f'TLE launch repeat selected ({len(repeat_launches)} objects total over {cfg["launchRepeatYrs"][1] - cfg["launchRepeatYrs"][0] + 1} years)')
            else:
                repeat_launches = np.zeros((0, mat_sats.shape[1]))
        else:
            # For 'random' model, start with empty repeat launches
            # In the future, this could be enhanced with Poisson-distributed random launches
            repeat_launches = np.zeros((0, mat_sats.shape[1]))
            print('Random launch model: using empty launches (can be enhanced with Poisson distribution)')
        ...
        if launch_model == 'matsat':
            ...
            print(f'TLE launch repeat selected ({np.sum(in_launch_window)} objects total over {cfg["launchRepeatYrs"][1] - cfg["launchRepeatYrs"][0] + 1} years)')
        else:
            repeat_launches = np.zeros((0, mat_sats.shape[1]))
    ...
    # Set constellation flags for Starlink and OneWeb (mass-based identification)
    constellation_mask = (mat_sats[:, idx['mass']] == 260) | (mat_sats[:, idx['mass']] == 148)
    ...
    mat_sats[controlled_mask, idx['a_desired']] = mat_sats[controlled_mask, idx['a']]
    ...
    cfg_out['repeatLaunches'] = repeat_launches if 'repeat_launches' in locals() else np.zeros((0, mat_sats.shape[1]))
    cfg_out['time0'] = time0
...
def create_default_satellites(n_sats: int, radiusearthkm: float) -> np.ndarray:
    ...
    mat_sats[:, 0] = (radiusearthkm + altitudes) / radiusearthkm  # Normalized to Earth radii
    ...
    mat_sats[:, 4] = np.random.uniform(0, 2*np.pi, n_sats)  # Argument of perigee
    ...
    mat_sats[:, 8] = (3 * volumes / (4 * np.pi))**(1/3)  # Spherical approximation
...
def fill_missing_physical_params(mat_sats: np.ndarray, idx: Dict) -> np.ndarray:
    ...
    if np.any(missing_mass):
        ...
        mat_sats[missing_mass & payload_mask, idx['mass']] = np.random.uniform(100, 5000, np.sum(missing_mass & payload_mask))
        
        # Rocket bodies: 500-10000 kg
        mat_sats[missing_mass & rb_mask, idx['mass']] = np.random.uniform(500, 10000, np.sum(missing_mass & rb_mask))
        
        # Other objects: 50-1000 kg
        other_mask = ~payload_mask & ~rb_mask
        mat_sats[missing_mass & other_mask, idx['mass']] = np.random.uniform(50, 1000, np.sum(missing_mass & other_mask))
    ...
    if np.any(missing_radius):
        ...
        mat_sats[missing_radius, idx['radius']] = (3 * volumes / (4 * np.pi))**(1/3)
